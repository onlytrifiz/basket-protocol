export type Country = {
  code: string;
  name: string;
  /** International dial prefix, used to show a realistic phone example. */
  dial: string;
};

/**
 * Every country the supplier returns a catalogue for, verified by probing
 * /v2/brands for each. Stock varies enormously — 562 brands in the US, ~21 in
 * the smallest territories — but all of these return something.
 *
 * Flags are derived from the ISO code rather than stored, so there is no second
...
const REGIONAL_INDICATOR = 0x1f1a5;

/** "IT" -> the Italian flag, via Unicode regional indicator symbols. */
export function countryFlag(code: string): string {
  ...
  return String.fromCodePoint(
    ...code
      .toUpperCase()
      .split("")
      .map((c) => REGIONAL_INDICATOR + c.charCodeAt(0)),
  );
}
...
export const COUNTRY_COOKIE = "stfy_country";
...
/** A plausible full number for the country, for input placeholders. */
export function phoneExample(code: string): string {
  const dial = BY_CODE.get(code)?.dial ?? "+1";
  return `${dial}3331234567`.slice(0, dial.length + 10);
}

export function dialCode(code: string): string {
  return BY_CODE.get(code)?.dial ?? "+1";
}


/** Human label for a supplier category key. */
export function categoryLabel(key: string): string {
  const map: Record<string, string> = {
    "e-commerce": "E-commerce",
    "e-money": "Prepaid & e-money",
    "e-sim": "eSIM",
    games: "Games",
    retail: "Retail",
    food: "Food & drink",
    ...
    mobile_credits: "Mobile credit",
    mobile_bundle: "Mobile bundles",
    mobile_talk_time: "Talk time",
    other_products: "Everything else",
  };
