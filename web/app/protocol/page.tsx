...
/* Four roles along one fee path; the vault is where value reaches holders. */
const roles = [
  { filled: 2, lit: false, title: "STFY", copy: "The 1B fixed-supply ERC-20 that maintains an on-chain registry of potential payout holders." },
  { filled: 4, lit: false, title: "FEE HOOK", copy: "A v4 hook that routes the 3% STFY trading fee, in ETH, to the dividend vault." },
  { filled: 6, lit: true, title: "DIVIDEND VAULT", copy: "The contract that accounts for each cycle and pushes acquired B20 stocks to eligible holders." },
  { filled: 8, lit: false, title: "KEEPER", copy: "The operational address that executes purchases and distribution batches." },
];

export default function ProtocolPage() {
  return <div className="site-shell"><SiteHeader active="protocol" /><main>
    <section className="page-intro wrap"><BrandRender className="page-intro-render" priority size={148} src="/logo.png" /><p className="eyebrow">PROTOCOL NOTES / 01</p><h1>Built to send<br /><em>stocks, not points.</em></h1><p>Stockify connects a Uniswap v4 trading fee to a configurable Base B20 equity index, then sends acquired assets to eligible STFY holders.</p></section>
    <section className="section wrap"><div className="section-head"><p className="eyebrow">ARCHITECTURE</p><h2>One fee path.<br />Four roles.</h2><p>The contracts separate collection, stock acquisition and payout accounting so each completed distribution has a clear on-chain record.</p></div><div className="roles-grid">{roles.map((role, index) => <article key={role.title}><RingMarker filled={role.filled} label={`0${index + 1}`} lit={role.lit} /><h3>{role.title}</h3><p>{role.copy}</p></article>)}</div></section>
    ...
    <section className="operation-card"><div className="operation-inner"><div><p className="eyebrow eyebrow-light">OPERATING MODEL</p><h2>Explicit about<br /><em>what is operated.</em></h2></div><div><p>Acquired B20 assets are retained by the dividend vault and payouts are carried out on-chain. A keeper executes stock purchases and batches; protocol ownership can update selected parameters and the active index between cycles.</p><p>Those roles are an operational dependency, not a claim of trustless automation. Deployed addresses and operating policy belong here before a live market opens.</p><Link className="button button-desk" href="/distributions">Distribution model <span>→</span></Link></div></div></section>
  </main><SiteFooter /></div>;
}
